...
Handles come from two producers: server-side generation, which lands in the
in-process FileStore, and agent upload, which lands in Postgres. Both resolve
...
async def test_build_files_from_handles_reads_filestore_bytes(
    ...
    assert len(files) == 1, "one handle should yield one attachment"
    fp = files[0].fp
    fp.seek(0)
    assert fp.read() == b"FAKEMP3", "generated bytes should come back unchanged"
