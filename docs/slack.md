...
Setup conversations can be opened with **💬 Set up with Daimon** in `/agent-setup`, an agent's details, or its creation confirmation. The public channel message starts a shared thread: Daimon answers there while the named agent is configured. Mention the bot to reply. Opening setup does not run a billed turn or change channel defaults. Each participant keeps a separate session.
